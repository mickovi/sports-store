    ...
    return class extends Base {
        storeProduct(p) {
            return this.sequelize.transaction(async (transaction) => {
                ...
                }, { transaction } // data will be write atomically,
                );
                return stored;
            });
        }
        async storeCategory(c, transaction) {
            const [stored] = await models_1.CategoryModel.upsert({ id: c.id, name: c.name }, { transaction });
            return stored;
        }
        async storeSupplier(s, transaction) {
            const [stored] = await models_1.SupplierModel.upsert({ id: s.id, name: s.name }, { transaction });
            return stored;
        }
    };
}
